    ...
    if request.method == "POST":
        ...
        if produto_obj:
            if current_user.compra_disponivel(produto_obj):
                ...
                flash(
                    f"Parabéns! Você comprou o produto {produto_obj.nome}!", category="success")
            else:
                flash(
                    f"Você não possui saldo suficiente para comprar o produto {produto_obj.nome}", category="danger")
        # Venda Produto        
        ...
        if produto_obj_venda: 
             if current_user.venda_disponivel(produto_obj_venda):
                 ...
                 flash(
                    f"Parabéns! Você vendeu o produto {produto_obj_venda.nome}!", category="success")
             else:
                flash(
                    f"Algo deu errado com a venda do produto {produto_obj_venda.nome}", category="danger")    
...
def page_login():
    ...
    if form.validate_on_submit():
        usuario_logado = User.query.filter_by(
            usuario=form.usuario.data).first()
        if usuario_logado and usuario_logado.converte_senha(senha_texto_claro=form.senha.data):
            login_user(usuario_logado)
            flash(
                f'Sucesso! Seu login é: {usuario_logado.usuario}', category='success')
            return redirect(url_for('page_produtos'))
        else:
            flash(f'Usuário ou senha estão incorretos! Tente novamente.',
                  category='danger')
